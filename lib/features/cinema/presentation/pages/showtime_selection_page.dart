...
class _ShowtimeSelectionPageState extends State<ShowtimeSelectionPage> {
              ...
              builder: (context, state) {
                if (state is CinemaLoading) {
                  ...
                  return ErrorStateWidget(
                    ...
                    onRetry: () => context.read<CinemaBloc>().add(
                          LoadShowtimesEvent(
                            movieId: widget.movieId,
                            date: DateTime.now(),
                          ),
  ...
  Widget _buildDateSelector() {
    return Container(
      height: 120, // Increased from 100 to 120 (8pt grid: 15A-8)
      padding: EdgeInsets.symmetric(vertical: AppDimens.spacing12),
      child: BlocBuilder<CinemaBloc, CinemaState>(
        builder: (context, state) {
          if (state is! ShowtimesLoaded) return const SizedBox.shrink();
